...
def decode_jwt(token: str, expected_type: str = "access") -> Dict[str, Any]:
    ...
    if not secret:
        raise TokenInvalidError(
            f"JWT signature verification failed: unknown kid '{kid}'"
        )
...
def validate_jwt_secret() -> None:
    ...
    for kid, secret in secrets_map.items():
        ...
        if len(secret) < 32:
            raise ValueError(
                f"JWT secret for kid '{kid}' must be at least 32 characters (256 bits)"
            )

    logger.info(
        f"JWT secrets validated successfully (loaded {len(secrets_map)} key(s))"
    )
