"""
Names Dictionary API — access to Uyghur person names, admin delete.
"""
...
@router.delete("/names-dictionary/{entry_id}", status_code=204)
async def delete_name_entry(
    entry_id: int,
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(require_admin),
):
    """Delete a names dictionary entry (Admin only)."""
    result = await session.execute(
        delete(NamesDictionary)
        .where(NamesDictionary.id == entry_id)
        .returning(NamesDictionary.id)
    )
