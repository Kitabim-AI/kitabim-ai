...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. Generate Content-Addressable Hash (Fingerprint)
      const hash = await generateFileHash(file);
      
      // 2. Query Global Document DB
      const existingBook = await PersistenceService.findBookByHash(hash);
      
      if (existingBook) {
        // MATCH FOUND: Skip OCR, just add to user's view (already in global books)
        setIsCheckingGlobal(false);
        refreshLibrary();
        return;
      }

      // 3. NO MATCH: Proceed with high-accuracy extraction
      setIsCheckingGlobal(false);
      const newBook: Book = {
      ...
      // Finalize and save to Global DB
      setBooks(prev => prev.map(b => {
        if (b.contentHash === hash) {
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider mt-4">
                      ...
                      <span className="text-indigo-600 font-black">READY</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex-grow bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                ...
                <div className="flex items-center gap-2">
                  {!isEditing ? (
                    <button onClick={() => setIsEditing(true)} className="btn-secondary flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors">
                    ...
                    <button onClick={saveCorrections} className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-100">
                      <Save size={16} /> Save to Global DB
                    </button>
                  )}
