/**
 * Names Dictionary Panel — read-only viewer for Uyghur person names.
 */
...
export const NamesDictionaryPanel: React.FC = () => {
  ...
  const fetchStats = async (group: string | null = activeGroup) => {
    try {
      ...
      const resp = await authFetch(`/api/names-dictionary/stats?${params}`);
      if (resp.ok) setStats(await resp.json());
    } catch (e) {
      console.error('Failed to fetch names dictionary stats', e);
    }
  };
  ...
  const fetchEntries = useCallback(
    async (pageNum: number, clear = false, group: string | null = activeGroup) => {
      ...
      try {
        ...
        if (group) params.set('letter_group', group);
        const resp = await authFetch(`/api/names-dictionary?${params}`);
        if (resp.ok) {
          const data: NameEntry[] = await resp.json();
          setAllEntries(prev => (clear ? data : [...prev, ...data]));
          setHasMore(data.length === PAGE_SIZE);
        }
      } catch (e) {
        console.error('Failed to fetch names dictionary entries', e);
      } finally {
  ...
  const searchEntries = async (q: string) => {
    ...
    try {
      const resp = await authFetch(
        `/api/names-dictionary/search?q=${encodeURIComponent(q)}&limit=10`,
      );
      if (resp.ok) setSuggestions(await resp.json());
    } catch (e) {
      console.error('Failed to search names dictionary', e);
    } finally {
    ...
    <div className="space-y-6 md:space-y-8 animate-fade-in pb-20" dir="rtl" lang="ug">
      ...
      <div className="flex flex-col-reverse md:flex-row items-center justify-between w-full gap-3 md:gap-4">
        <div className="relative flex-1 lg:flex-none lg:w-[40%] group w-full">
          <div className="absolute inset-y-0 right-4 md:right-5 flex items-center pointer-events-none text-[#0369a1] z-10 font-bold">
            {isSearching ? (
          ...
          <input
            ...
            placeholder={t('admin.namesDictionary.searchPlaceholder')}
            dir="rtl"
          />
          {searchQuery && (
        ...
        {stats && (
          <div className="flex items-center gap-2 text-[12px] md:text-[14px] font-normal text-[#0369a1] bg-[#0369a1]/10 px-3 md:px-4 py-2 md:py-2.5 rounded-full border border-[#0369a1]/20 shadow-sm whitespace-nowrap self-end md:self-auto md:mr-auto">
            ...
            {t('admin.namesDictionary.totalEntries', {
          ...
          <div className="glass-panel rounded-[24px] md:rounded-[32px] py-8 md:py-12 px-4 md:px-8 flex flex-col items-center justify-center gap-3 md:gap-4 text-center animate-fade-in shadow-lg border border-[#0369a1]/10">
            ...
            <div className="space-y-1">
              <h3 className="text-lg md:text-xl font-normal text-[#1a1a1a]">
                {t('admin.namesDictionary.entryNotFound')}
              </h3>
            ...
            <div className="glass-panel rounded-[32px] p-2 overflow-hidden shadow-xl animate-fade-in border border-[#0369a1]/5">
              <div className="grid grid-cols-1 gap-1">
                {activeEntries.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 rounded-2xl transition-all group hover:bg-slate-50/50"
                  >
                    <span className="uyghur-text text-[16px] md:text-xl font-bold text-slate-800">
                      {entry.name}
                    </span>
                    <span className="uyghur-text text-[12px] md:text-[13px] text-[#0369a1]/60 bg-[#0369a1]/5 px-2.5 py-0.5 rounded-full border border-[#0369a1]/10">
                      {entry.letter_group}
                    </span>
                  </div>
                ))}
