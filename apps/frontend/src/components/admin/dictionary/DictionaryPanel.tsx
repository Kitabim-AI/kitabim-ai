/**
 * Dictionary Panel - Editor/Admin tool to view definitions dictionary (Read-Only)
 */
...
export const DictionaryPanel: React.FC = () => {
  ...
  const fetchStats = async (group: string | null = activeGroup) => {
    try {
      ...
      const resp = await authFetch(`/api/dictionary/stats?${params}`);
  ...
  const fetchAllEntries = useCallback(async (pageNum: number, clear: boolean = false, group: string | null = activeGroup) => {
    ...
    try {
      ...
      if (group) params.set('letter_group', group);
      const resp = await authFetch(`/api/dictionary?${params}`);
      if (resp.ok) {
  ...
  const searchEntries = useCallback(async (q: string) => {
    ...
    setIsSearching(true);
    try {
      const resp = await authFetch(`/api/dictionary/search?q=${encodeURIComponent(trimmed)}&limit=10`, {
  ...
  const handleDeleteEntry = (entry: DictionaryEntry) => {
    setModal({
      ...
      message: t('admin.dictionary.confirmDelete', { word: entry.word }),
      type: 'confirm',
      ...
      onConfirm: async () => {
        ...
        try {
          const resp = await authFetch(`/api/dictionary/${entry.id}`, { method: 'DELETE' });
          ...
          addNotification(t('admin.dictionary.deleteSuccess'), 'success');
        } catch (e) {
          console.error('Failed to delete dictionary entry', e);
          setModal((prev: any) => ({ ...prev, isOpen: false }));
          addNotification(t('admin.dictionary.deleteError'), 'error');
        }
      },
    });
  };
              ...
              <div className="grid grid-cols-1 gap-1">
                  {activeEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className={`
                        flex items-start justify-between px-4 md:px-6 py-4 md:py-5 rounded-2xl transition-all group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 gap-3
                      `}
                    >
