...
import { Database, ChevronUp, ChevronDown, Tag, X, Save, Trash2, BookType, User, Hash, BookOpen, Cpu, ScanText, History, RotateCcw, RefreshCw, LayoutDashboard, ScanSearch } from 'lucide-react';
import { OcrRegistryPanel } from './OcrRegistryPanel';
...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const [activeTab, setActiveTab] = React.useState<'books' | 'registry'>('books');
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'books'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'registry'
              ? 'bg-white text-indigo-600 shadow-sm'
              : 'text-slate-500 hover:text-slate-700'
              }`}
          >
      ...
      {activeTab === 'registry' ? (
        <OcrRegistryPanel onOpenReader={onOpenReader} books={books} />
      ) : (
        <>
          {isCheckingGlobal && (
