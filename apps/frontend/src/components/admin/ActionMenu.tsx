...
import { BookOpen, Shield, Globe, ScanText, RotateCcw, RefreshCw, Trash2 } from 'lucide-react';
...
export const ActionMenu: React.FC<ActionMenuProps> = ({ book, close }) => {
  ...
  return (
    <div className="absolute left-0 top-full mt-2 w-52 glass-panel shadow-2xl z-50 py-2 rounded-[16px] border border-[#0369a1]/10">
      ...
      <button onClick={() => { bookActions.handleToggleVisibility(book.id, book.visibility || 'public'); close(); }} className="w-full flex items-center gap-3 px-4 py-2 text-sm font-black text-[#1a1a1a] hover:bg-[#0369a1]/10 transition-colors">{book.visibility === 'private' ? <><Shield size={14} /> {t('admin.table.makePublic')}</> : <><Globe size={14} /> {t('admin.table.makePrivate')}</>}</button>
