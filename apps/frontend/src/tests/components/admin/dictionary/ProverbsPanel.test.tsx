...
import { ProverbsPanel } from '@/src/components/admin/dictionary/ProverbsPanel';
...
vi.mock('@/src/hooks/useAuth', async () => {
  ...
  return { ...(actual as any), useIsAdmin: vi.fn(), useIsEditor: vi.fn() };
});
...
const mockEntry = { id: 1, text: 'ياخشى ئىش قىلساڭ ياخشى كۆرىسەن' };
...
function mockListResponses() {
  vi.mocked(authService.authFetch).mockImplementation(async (url: string, opts?: any) => {
    ...
    if (url.includes('/api/proverbs/stats')) {
      return { ok: true, json: async () => ({ total_entries: 1 }) } as Response;
    }
    if (url.includes('/api/proverbs?')) {
...
test('does not show a delete button for non-admin users', async () => {
  ...
  render(<ProverbsPanel />);

  await screen.findByText(mockEntry.text);
  expect(screen.queryByTitle('common.delete')).not.toBeInTheDocument();
});

test('admin can delete a proverb after confirming', async () => {
  ...
  expect(setModal).toHaveBeenCalledWith(
    expect.objectContaining({
      ...
      type: 'confirm',
      destructive: true,
      message: 'admin.proverbs.confirmDelete',
    })
  );
  ...
  expect(authService.authFetch).toHaveBeenCalledWith('/api/proverbs/1', { method: 'DELETE' });
  await waitFor(() => expect(screen.queryByText(mockEntry.text)).not.toBeInTheDocument());
});
