...
import { EnglishUyghurPanel } from '@/src/components/admin/dictionary/EnglishUyghurPanel';
...
const mockEntry = { id: 1, english: 'book', uyghur: 'كىتاب', letter_group: 'B' };
...
function mockListResponses() {
  vi.mocked(authService.authFetch).mockImplementation(async (url: string, opts?: any) => {
    ...
    if (url.includes('/api/english-uyghur-dictionary/stats')) {
      return { ok: true, json: async () => ({ total_entries: 1 }) } as Response;
    }
    if (url.includes('/api/english-uyghur-dictionary?')) {
...
test('admin can delete an entry after confirming', async () => {
  ...
  expect(setModal).toHaveBeenCalledWith(
    expect.objectContaining({
      ...
      type: 'confirm',
      destructive: true,
      message: 'admin.englishUyghurDictionary.confirmDelete',
    })
  );
  ...
  expect(authService.authFetch).toHaveBeenCalledWith('/api/english-uyghur-dictionary/1', { method: 'DELETE' });
  await waitFor(() => expect(screen.queryByText('book')).not.toBeInTheDocument());
});
