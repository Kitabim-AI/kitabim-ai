...
import { SynonymsPanel } from '@/src/components/admin/synonyms/SynonymsPanel';
...
const mockEntry = { id: 1, word: 'كىتاب', letter_group: 'ك', synonyms: ['ئەسەر'] };
...
function mockListResponses() {
  vi.mocked(authService.authFetch).mockImplementation(async (url: string, opts?: any) => {
    ...
    if (url.includes('/api/synonyms/stats')) {
      return { ok: true, json: async () => ({ total_words: 1 }) } as Response;
    }
    if (url.includes('/api/synonyms?')) {
...
test('admin can delete an entry after confirming', async () => {
  ...
  expect(setModal).toHaveBeenCalledWith(
    expect.objectContaining({
      ...
      type: 'confirm',
      destructive: true,
      message: 'admin.synonyms.confirmDelete',
    })
  );
  ...
  expect(authService.authFetch).toHaveBeenCalledWith('/api/synonyms/1', { method: 'DELETE' });
