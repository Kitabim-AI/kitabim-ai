...
The query assistant uses an agentic RAG loop (`AgentRAGHandler`, priority=998) as the sole handler for all content questions. The assistant is built using a stateless Google ADK `Agent` run via `InMemoryRunner` for each chat request. 
...
|---------|----------|-----------|
| `AgentRAGHandler` | 998 | Sole handler — all user chat questions run this handler |
