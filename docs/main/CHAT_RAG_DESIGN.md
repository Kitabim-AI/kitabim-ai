...
`use_adk_chat_v2`, `use_deterministic_router`, and `rag_eval_enabled` no longer exist — they selected between pipelines/handlers that have since been deleted (see the prior design note above). `agent_max_steps` and `agent_enough_chunks` were never true feature flags (nothing consumed the `QueryContext` fields they populated) and are also gone.
