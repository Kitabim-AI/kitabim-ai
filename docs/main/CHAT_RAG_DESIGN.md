...
| `rag_hybrid_search_enabled` | `"true"` (`app/db/seeds.py`) | `vector_search()` fuses a Postgres full-text `keyword_search` leg with the pgvector leg via Reciprocal Rank Fusion. `"false"` = vector-only. Applies to both pipelines. |
...
| `packages/backend-core/app/services/rag/agent/config.py` | Numeric constants: `AGENT_MAX_STEPS`, `AGENT_ENOUGH_CHUNKS`, `AGENT_MAX_CONTEXT_CHUNKS`, `GRADE_RELATIVE_THRESHOLD`, `MIN_CHUNKS_AFTER_GRADING`, `RERANK_MAX_INPUT_CHUNKS`, `RRF_K`, `CONTEXT_SWITCH_SCORE_THRESHOLD`. |
...
flowchart TD
    ...
    subgraph Orchestrator ["ChatOrchestrator — orchestrator.py"]
        ...
        RR["rerank_context<br/>(LLM, max_chunks = rag_top_k)"]
        GC1["_grade_context<br/>(max_chunks = rag_top_k)"]
    ...
    class Q,LIMIT,EP,V2,RERANK,SEL,JUDGE,EVAL idle
    class OCTX,SIG,RETR,RR,GC1,ANSA,ENQ,RCTX,DET,LLMR,GC2,SYN,TOOLS,VS,WORKER,INC active
    class CONV,PERSIST,REC,DATA,OUT,NOREC done
    class L429 fail
```
...
4. signals = DeterministicRAGHandler()._llm_analyze_query(question, ctx);
   yield {"type":"planning","intent": signals["intent"] or "open"}.
   (Runs on every request; independent of use_deterministic_router.)
...
6. retrieval_agent = build_retrieval_agent(agent_model, signals). Runner:
   the app-level ADK DatabaseSessionService when present (session id =
   conv_id, created if absent), else a fresh InMemorySessionService.
   Seed adk_session.state["query_context"]=ctx, ["observations"]=[].
7. Run runner.run_async(new_message=_build_human_message(ctx, question)
   wrapped as types.Content, RunConfig(streaming_mode=SSE)). The
   [Context] block from _build_human_message is required — without it the
   agent cannot tell it is in reader mode. From the event stream, yield
   tool_call / agent_thinking on non-partial events, and for every
   function response append {"tool","result"} to a local observations
   list and yield tool_result with result["found_count"].
8. used_book_ids = _extract_used_book_ids(observations).
   rag_top_k = int(system_configs "rag_top_k", default str(settings.rag_top_k)).
...
9. yield {"type":"answer_start"}; build_answer_agent(chat_model,
   graded_context, persona_prompt, is_global, has_categories) and run it
   through the same shared session service (or an InMemoryRunner),
   yielding {"type":"chunk","text"} per partial part; fall back to
   non-partial parts if no partial events arrived. yield answer_end.
10. fixed_text = fix_malformed_citations(accumulated_text).
11. create_evaluation(... retrieved_count=len(observations),
    context_chars=len(graded_context), scores=[1.0]*len(observations)
    (placeholders, not real similarities), category_filter=
    request context_book_ids, agent_steps=len(observations),
...
7. graded_context, before, after = _grade_context(observations) — with NO
   ...
   constant (25), not the rag_top_k system config.
...
`_path_open` passes **all** book IDs returned by `search_books_by_summary` (up to its `limit=20`) to `search_chunks`; only `get_book_summary` is capped at 5. `vector_search` gives each book its own quota — `max(rag_top_k // len(book_ids), 3)` — so a book-level cutoff here would drop legitimately relevant books rather than prevent dilution.
...
2. Per search_chunks observation, in isolation: build Documents, sort by
   ...
   rrf_score > 0 OR a keyword rank (so keyword-only hybrid hits survive).
...
**Shared retrieval — `vector_search(ctx, book_ids, query_vector)` (`retrieval.py`), reached by every `search_chunks` call from any pipeline:**
...
2. Read rag_hybrid_search_enabled and rag_top_k from system_configs
   (falling back to settings.rag_top_k).
...
4. For more than one book, run one _search_chunks per book concurrently
   with per_book_limit = max(rag_top_k // n_books, 3) and merge; a single
   global top-K would let the highest-scoring book crowd the others out.
   Otherwise one call with limit=rag_top_k.
5. _search_chunks = ChunksRepository.similarity_search(threshold=
   settings.rag_score_threshold) plus, when hybrid is on, a parallel
   ChunksRepository.keyword_search fused by _fuse_rrf
   (score = Σ 1/(RRF_K + rank), RRF_K = 60). A failing keyword leg falls
   back to vector-only for that one call. The RRF score is returned as a
   separate rrf_score field and never overwrites `similarity`, because
   downstream absolute-threshold checks (CONTEXT_SWITCH_SCORE_THRESHOLD)
   are calibrated for genuine cosine similarity.
6. IF zero chunks came back, retry the same shape with threshold=0.0.
7. Dev-only: still zero, book_ids present, and settings.environment !=
...
8. IF is_islam_or_quran_query(question or enriched_question): pgvector
...
| Hybrid-search keyword leg errors (e.g. malformed `tsquery`) | Logged; that one `_search_chunks` call returns vector-only results. Not a turn failure. |
...
| Key | Default | Used by |
|---|---|---|
| `rag_top_k` (`system_configs`) / `settings.rag_top_k` (env `RAG_TOP_K`, `app/core/config.py`) | `"25"` seeded in `app/db/seeds.py`; env/code default **`25`** (verified in `config.py`) | `vector_search` (per-call `limit`, and `max(rag_top_k // n_books, 3)` per book on multi-book searches; also the Quran merge cap) and `ChatOrchestrator`'s `max_chunks` for rerank/grade. The DB key wins; `settings.rag_top_k` is the fallback when the key is absent or unparseable. **Not** consulted by the registry-path handlers' own `_grade_context` calls. |
...
| `settings.summary_threshold` (env `SUMMARY_THRESHOLD`) | `0.45` | `_run_search_books_by_summary` → `BookSummariesRepository.summary_search(threshold=...)`; the book-routing cut-off. |
...
| `RRF_K` (`rag/agent/config.py`) | `60` | Reciprocal Rank Fusion constant in `_fuse_rrf`. |
...
| `ChatRequest` validators (`app/models/schemas.py`) | 500 chars; must contain at least one Arabic-script codepoint | Rejects the request with a 422 before any LLM call. |
...
| L2 | `rag:search:{book_id}:{hash}` / `rag:search:multi:{book_ids_hash}:{hash}` (`KEY_RAG_SEARCH_SINGLE` / `KEY_RAG_SEARCH_MULTI`) | `vector_search` (never for dev-only fuzzy-fallback results) | Cache pgvector/hybrid results. |
...
- **Prompt-injection surface: book content flows into LLM context.** OCR'd book text is untrusted input from a third-party document, and it reaches the model verbatim through `search_chunks` → `_grade_context`/`rerank_context` → the answer prompt, and through `get_current_page` (raw `pages.text`, markdown-stripped). There is no sanitization, escaping, or instruction-hierarchy marker on retrieved passages; the only structural separation is the `[BookID: …, Book: …, Page: N]` header `format_document` prepends and the `\n\n---\n\n` joiner. Three consequences are worth naming explicitly:
  ...
  - Tool *arguments* are model-chosen, but no tool takes free-form SQL. Every retrieval path goes through SQLAlchemy bound parameters, including the raw-SQL Quran query in `retrieval.py` (`sa_text(...)` with `:embedding` / `:limit` bind params) and the category filter (`sa_text("categories && CAST(:cats AS text[])").bindparams(...)`). `book_ids` from the model are coerced with `str()` and used only as bound `IN` values.
...
- `rag_retrieval_test.py` — `retrieval.py` primitives (embedding cache, hybrid fusion, scoping).
- `rag_service_caching_test.py` — L1/L2 cache key construction and hit/miss behavior.
- `test_retrieval_subset_matching.py` — `find_books_by_title_in_question`: `test_find_books_by_title_subset_filtering`, `test_find_books_by_title_quoted_multiple_titles`, `test_find_books_by_title_quoted_no_match_returns_none`, `test_find_books_by_title_orders_by_volume`.
...
- [SUMMARY_DESIGN.md](SUMMARY_DESIGN.md) — produces `book_summaries`, which this stage consumes through `search_books_by_summary` (vector search over summary embeddings, `settings.summary_threshold` = 0.45, `limit=20`) and `get_book_summary` (full summary text, server-side sister-volume expansion, intro-excerpt fallback when no summary row exists). This is the "which book(s) is the question about" step that precedes chunk retrieval.
...
- [CHUNKING_DESIGN.md](CHUNKING_DESIGN.md) — defines the chunk boundaries and `chunks.text` that become the citable passages, and the TOC exclusion this stage also honors.
