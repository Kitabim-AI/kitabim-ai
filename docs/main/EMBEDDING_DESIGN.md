...
- **The reactive `EventDispatcher` path is interactive-only.** Unlike `EmbeddingScanner`, `EventDispatcher`'s `chunking_succeeded`-triggered dispatch always enqueues `embedding_job` directly — it does not check `embed_batch_enabled` or ever submit a batch job. So when batch mode is enabled, a page only goes through the batch path if `EmbeddingScanner`'s poll claims it before the dispatcher does. Both are registered in `services/worker/worker.py` as plain `cron(fn)` with no `minute`/`second` override, which arq's `next_cron` resolves to the identical once-per-minute (`second=0`) schedule for both — so despite the "reactive" name, `EventDispatcher` is not a sub-minute or pub/sub-triggered path (no `LISTEN`/`NOTIFY` or Redis pub/sub wake-up exists on `chunking_succeeded`); its only scheduling edge over `EmbeddingScanner` is `run_at_startup=True` (one extra run when the worker process boots). At each shared per-minute tick, whichever of the two `SKIP LOCKED` claims commits first for a given page wins — not a fixed "seconds" head start — so which path a page takes when batch mode is enabled is effectively non-deterministic per tick, not dispatcher-favored.
...
| Column | Type | Description |
|---|---|---|
| `embedding` | `vector(3072)`, nullable | pgvector embedding; `NULL` until this stage fills it in (or until `ChunkingJob` resets it to `NULL` on a text change — see [CHUNKING_DESIGN.md](CHUNKING_DESIGN.md)). Filled in by `EmbeddingJob` (`UPDATE chunks SET embedding = ...`) or, in batch mode, by `batch_embedding_service.poll_and_process_batch_embedding_jobs` once the Gemini Batch API job succeeds. Indexed by `idx_chunks_embedding` — an `HNSW` index (`m=16, ef_construction=64`) over `embedding::halfvec(3072)` using `halfvec_cosine_ops`, created (as `idx_chunks_embedding_v2`) by migration 036 and renamed to its current name by migration 037; used by RAG's `ChunksRepository.similarity_search` (out of scope here), not by this stage's writes. |
...
| `services/worker/scanners/event_dispatcher.py` | `run_event_dispatcher` — claims and dispatches `embedding_job` off `chunking_succeeded` outbox events, interactive-only (see Overview). Runs on the same once-per-minute arq cron cadence as `EmbeddingScanner` (plus once at worker startup) — not a lower-latency trigger. |
...
| `embed_gemini_timeout` (`system_configs`) | `"15"` (seeded) | Seeded and shown in the admin System Configs panel as "Timeout in seconds for Gemini vector embedding API calls," but **not actually read by the interactive embedding call path**: `GeminiEmbeddings.aembed_documents`/`aembed_query` (`packages/backend-core/app/llm/models.py`) hardcode `aiohttp.ClientTimeout(total=15.0)` inline, and the enclosing `_call_with_breaker(_EMBED_BREAKER, ...)` call passes no `timeout`, so it falls back to the unrelated hardcoded `_INVOKE_TIMEOUT` (60s). A `_BREAKER_TIMEOUT_CONFIGS["llm_embed"] → "embed_gemini_timeout"` mapping exists for `get_system_config_timeout()`, but that lookup only fires inside `ProtectedLLM`, which is only ever constructed with `_TEXT_BREAKER` — never `_EMBED_BREAKER` — so this key is currently inert for embeddings (its default happens to match the hardcoded 15s value by coincidence). |
