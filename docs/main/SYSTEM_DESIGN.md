...
- **Gemini Infrastructure**
  - Interactive (real-time) API for embeddings, chat, summarization, and entity extraction — the default path for these.
  - Gemini **Batch API**, feature-flagged per pipeline stage, for lower-cost high-volume processing at the expense of latency (async submit + poll instead of an immediate response): OCR (`ocr_batch_enabled`, default `true` — Batch is now the out-of-the-box OCR path), embedding (`embed_batch_enabled`, default `false`), history-term extraction (`history_batch_enabled`, default `false`), and on-demand LLM spell-check (`llm_spell_check_batch_enabled`, default `false`).
...
- `llm_spell_check_status` (`idle`/`in_progress`/`succeeded`/`failed`) and `llm_spell_check_at`: track the separate, admin-triggered on-demand Gemini spell-correction pass (see §6F) — independent of the automatic `spell_check_milestone` above.
...
**`batch_ocr_jobs` / `batch_embedding_jobs` / `batch_history_extraction_jobs` / `batch_llm_spell_check_jobs`**
- One row per Gemini Batch API submission (`gemini_batch_id`, `status` — `submitting | submitted | running | succeeded | failed | cancelled`, GCS input/output URIs, page/book/chunk ID arrays; `batch_llm_spell_check_jobs` uses a simpler shape — serial `id`, single `book_id` FK, `page_ids[]`, `total_batches`). Written by `batch_ocr_service.py`/`batch_embedding_service.py`/`batch_history_extraction_service.py`/`batch_llm_spell_check_service.py` when the corresponding `ocr_batch_enabled` / `embed_batch_enabled` / `history_batch_enabled` / `llm_spell_check_batch_enabled` system config is on, and updated by the four dedicated poller scanners as Gemini's batch job progresses. `ocr_batch_enabled` defaults to `true`; the other three default to `false` and their tables only exist once batch mode has been used at least once.
...
**Platform tables**: `users` (role: `admin` | `editor` | `reader`), `refresh_tokens`, `user_chat_usage` (daily chat-quota tracking), `rag_evaluations` (per-request RAG telemetry, optionally linked to a `conversation_id` — including per-turn `input_tokens`/`output_tokens`/`cost_usd` and a nullable `judge_cost_usd` backfilled after async LLM-judge scoring completes), `system_configs` (runtime-tunable settings, see below), `contact_submissions`.
...
A key/value table (seeded with defaults, editable via the admin dashboard) that drives model selection and pipeline tuning without a redeploy — including `rag_gemini_chat_model`, `ocr_gemini_model`, `embed_gemini_model`, `kg_gemini_extraction_model`, `gemini_agent_loop_model` (optional override, otherwise falls back to `rag_gemini_chat_model`), `history_gemini_model`, `gemini_llm_spell_check_model` (default `gemini-3.1-flash-lite`), `rag_reranker_enabled`, `rag_judge_scoring_enabled`, `rag_chat_cost_enabled` (show/hide per-turn token/cost info in the chat UI, default `true`), `kg_enabled`, `history_extraction_enabled`, `ocr_batch_enabled` (default `true`) / `embed_batch_enabled` / `history_batch_enabled` / `llm_spell_check_batch_enabled` (the latter three default `false`), and various batch-size/timeout/retention knobs.
...
2. **OCR**: the OCR scanner leases `idle` pages. Since `ocr_batch_enabled` defaults to `true`, the OCR job normally submits the claimed pages as a Gemini Batch API job (`batch_ocr_jobs` row) and returns immediately, with a dedicated poller scanner picking up the result asynchronously (see [WORKER_DESIGN.md](WORKER_DESIGN.md)). When `ocr_batch_enabled` is turned off, the job instead renders each page to an image and calls Gemini Vision directly (via `google-genai`) using the `ocr_gemini_model` config, writing to `pages.text` and setting `ocr_milestone` to `succeeded` in the same call.
...
2. Checks first for a deterministic **short-work request**: `detect_short_work_intent` (`rag/short_work_intent.py`) flags quoted, "locate/show the full text of" style questions (Uyghur locate-verb markers, 1–2 quoted spans, no authorship marker) with no LLM call. If it applies, `resolve_short_work` (`rag/toc_lookup_service.py`) resolves the quoted title against the book's OCR'd table of contents (`Page.is_toc`, `parse_toc_entries`), bounds a page range against the next ToC entry (capped by `toc_short_work_max_pages`, default 10), and verifies the resolved start page. On a match, the turn skips signal extraction, the retrieval agent, reranking, and the answer agent entirely — the answer is assembled directly from the ToC-bounded page text (`_format_short_work_answer`), since Gemini otherwise tends to hedge on reproducing full texts verbatim.
3. Otherwise, skips retrieval entirely for exact-phrase/quoted questions (`detect_phrase_intent`, `rag/phrase_intent.py`): a keyword-only leg (`chat/exact_phrase.py`, via `rag/retrieval.py`'s `exact_phrase_chunk_search` → `ChunksRepository.keyword_search`) answers directly from `pages.text_search` matches, rendering page-finding phrasing ("find pages with...") as raw page hits instead of an LLM-synthesized answer.
4. Otherwise (neither the short-work nor the exact-phrase gate applied), loads the last 6 messages of that conversation as pre-processing context, and runs a single-shot structured Gemini signal-extraction call (`analyze_query_signals`, `chat/query_signals.py`) that classifies intent (`catalog`, `dictionary`, `identity`, `summary`, `relationship`, `passage`, `quran`), pronoun-coreference needs, and volume-shift requests, alongside DB lookups for title/author matches. The extracted `intent` is read directly by the orchestrator, and so are the catalog-related fields consumed by step 5 below; the rest feeds the retrieval agent's prompt hints.
5. Checks the just-extracted signal for a clean **`author_of`/`books_by` catalog match** (`_resolve_catalog_shortcut`, `orchestrator.py`) — reusing the `find_books_by_title`/`get_books_by_author` DB lookups step 4's own call already made, not a new detection pass. When the match is unambiguous (a single matched title for `author_of`, or any author-matched books for `books_by`) and the question isn't composite, the answer is templated directly (`_format_catalog_answer`) with no LLM call, skipping the retrieval agent, reranking, and the answer agent entirely — the same style of shortcut as the short-work path in step 2, added because the retrieval agent could already answer these via its own `get_book_author`/`get_books_by_author` tools, but only after two full LLM turns the signal-extraction call's own tool calls had already made unnecessary. Anything ambiguous or unresolved falls through to step 6 unaffected.
6. Otherwise, builds a **retrieval agent** (`chat/retrieval_agent.py`) — `AGENT_SYSTEM_PROMPT` over all 20 tools (`services/rag/agent/tools.py`: passage search, summary-based book discovery, knowledge-graph lookup, catalog/author/title/volume metadata tools, per-page retrieval, query rewriting, and dictionary/proverb/name/spelling/Quran lookups) — and runs it via an ADK `Runner` backed by a persistent `DatabaseSessionService` (wired in `services/backend/main.py`), streaming `tool_call`/`tool_result`/`agent_thinking` events.
7. Grades the collected context — by default via an LLM reranker (`rerank_context`, gated by `rag_reranker_enabled`, default `true`) that replaces the relative-score selection with real semantic reranking, falling back to `_grade_context` (`chat/context_grading.py`) if the reranker call fails, times out, or is disabled — then extracts used book IDs with `_extract_used_book_ids`. Skipped for the short-work path (step 2) and the catalog-shortcut path (step 5).
8. Builds a separate, tools-less **answer agent** (`chat/answer_agent.py`) to stream the final answer, using its own citation-instruction prompt (`chat/answer_prompts.py`). Skipped for the short-work path (step 2) and the catalog-shortcut path (step 5), both of which format their answer directly instead.
9. Persists both the user and model messages via `ConversationRepository.save_turn()`, writes a `rag_evaluations` row linked to the conversation — including per-turn LLM token/cost usage (`input_tokens`, `output_tokens`, `cost_usd`, tracked across the signal-extraction, retrieval agent, reranker, and answer agent calls via `CostTracker`/`app/llm/cost_tracker.py` and `app/llm/pricing.py`) — enqueues `rag_eval_job` for async LLM-judge scoring of the turn (gated by `rag_judge_scoring_enabled`, default `true`, which also backfills a `judge_cost_usd` column once scoring completes), and emits a `done` event carrying `conversationId` — on every turn, streaming or not.
...
### F) LLM Spell Check (admin-triggered)

A second, independent spell-check layer: an on-demand Gemini-based correction pass, distinct from the automatic dictionary/auto-correct-rule pipeline (`spell_check_milestone`, §6A step 5) — this one is never scanner-driven and runs only when an admin asks for it. `POST /api/books/{book_id}/reprocess/llm-spell-check` (`require_admin`, since each trigger costs real Gemini API calls) queues every eligible page in a book; `POST /api/books/{book_id}/pages/{page_num}/llm-spell-check` triggers a single page and always uses the live path. The live path enqueues `llm_spell_check_job` (`services/worker/jobs/llm_spell_check_job.py`, concurrency-bounded by `settings.max_parallel_llm_spell_check`, default 6 — an app setting, not a `system_configs` row); it is not a pipeline stage — no `pipeline_events` rows, no `retry_count` bookkeeping, no scanner-driven page claiming. `llm_spell_check_service.correct_page_text` sends each page's text plus its neighboring pages as context to `gemini_llm_spell_check_model` (default `gemini-3.1-flash-lite`), validates the correction isn't empty or implausibly different in length from the original, and overwrites `pages.text` on success; `pages.llm_spell_check_status`/`llm_spell_check_at` track per-page progress. If `llm_spell_check_batch_enabled` is on (default `false`), the book-level trigger instead submits pages as a Gemini Batch API job (`batch_llm_spell_check_jobs` row via `batch_llm_spell_check_service.py`), picked up asynchronously by `batch_llm_spell_check_poller_scanner`.

## 7) Gemini Integration Strategy
- **`google-genai` SDK** — used for every direct (non-agentic) AI call: the File API for OCR image uploads, OCR text extraction, book summarization, structured knowledge-graph entity/relation extraction (Pydantic schemas), history-term extraction and definition synthesis, on-demand LLM spell correction, embedding generation, and ChatOrchestrator's signal-extraction and query-rewrite calls.
- **Google ADK (`google-adk`)** — used for `ChatOrchestrator`'s two-agent pipeline: a free-form ReAct retrieval `Agent` over the 20-tool registry, and a tool-less answer `Agent` for synthesis, both run through an ADK `Runner`.
...
- **Batch job resilience**: the `batch_ocr_jobs` poller scanner enforces a wall-clock timeout (`ocr_batch_timeout_hours`, default 24h); the `batch_embedding_jobs` poller scanner enforces both a timeout (`embed_batch_timeout_hours`, default 24h) and a retry budget (`embed_batch_max_retry_count`, default 3) per job — both fall back to marking affected pages failed rather than blocking indefinitely. `batch_history_extraction_jobs`/`batch_history_poller_scanner` and `batch_llm_spell_check_jobs`/`batch_llm_spell_check_poller_scanner` reuse the same submit-then-poll shape but do not yet have equivalent timeout/retry-count system configs.
...
- **Feedback & telemetry**: `rag_evaluations` captures per-request metrics, per-turn LLM token/cost usage (`input_tokens`/`output_tokens`/`cost_usd`, with an async-backfilled `judge_cost_usd`; surfaced in the chat UI when `rag_chat_cost_enabled` is on), and thumbs-up/down feedback for manual review; `ChatOrchestrator` turns additionally get an async LLM-judge score (`rag_eval_job`) — there is no corpus-wide/batch offline evaluation job.
...
- OAuth login via Google, Facebook, and Twitter/X, with PKCE for Twitter and an httpOnly refresh-token cookie. An Instagram provider class exists (`auth/providers/instagram_provider.py`) but is not registered in the provider registry, not exempted in the app-ID enforcement middleware, and not wired into the frontend — effectively dormant.
