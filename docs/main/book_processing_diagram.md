...
Visual companion to [WORKER_DESIGN.md](WORKER_DESIGN.md). All Gemini API calls are synchronous/real-time — no Gemini Batch API is used anywhere in the pipeline.
...
| Component | Role |
|---|---|
| **ARQ Worker** | Executes the 7 registered jobs and 11 scheduled scanners via the Redis-backed queue |
...
| **MultiPageLock** | Redis-backed per-page lock (1‑hour expiry) used by `ocr_job`, `chunking_job`, `embedding_job`, and `spell_check_job` to prevent double-processing of a claimed page |
