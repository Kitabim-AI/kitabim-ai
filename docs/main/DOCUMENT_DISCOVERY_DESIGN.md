...
Document Discovery is how a book enters the Kitabim.AI pipeline. There are three independent entry points, and all converge on the same outcome — a `Book` row plus its `Page` rows:
...
- **Pre-OCR'd PDF ingestion** — an editor/admin (in practice, the standalone `clients/kitabim-ocr` local Surya OCR client) calls `POST /books/upload-ocrd` with a `.pdf` file plus a `pages` part carrying already-OCR'd page text. Handled inline in the request (`upload_pdf_ocrd` in `services/backend/api/endpoints/books_router.py`); the client itself is out of this doc's scope, but the backend endpoint it calls is a document-discovery entry point.

Key characteristics:

- **Duplicate detection is two-layered**: a fast filename/ID check (DB-only, no download) followed by a SHA-256 content-hash check (`books.content_hash` has a unique DB constraint, so even a race between two concurrent inserts is caught by an `IntegrityError` rather than producing duplicate rows). The pre-OCR'd upload path only performs the content-hash check (it has no filename/ID pre-check, since it isn't scanning a bucket).
- **PDF vs. DOCX vs. pre-OCR'd PDF diverge after creation**: plain PDF uploads (manual upload and GCS discovery) create empty `Page` stubs and enter the pipeline at OCR. DOCX uploads (manual upload only — the GCS scanner only discovers `.pdf` files) and pre-OCR'd PDF uploads (`POST /books/upload-ocrd`) both extract/receive page text immediately and skip OCR entirely, entering the pipeline at chunking.
- **This stage does not trigger OCR itself.** All three paths only create rows; `PipelineDriver` (documented in `WORKER_DESIGN.md`) is what initializes idle pages into the OCR step on its next run.
- **No feature flag gates this stage** — none of the upload endpoints nor `GcsDiscoveryScanner` checks a `system_configs` flag before running.
...
|---|---|---|
| `id` | `varchar(64)` | GCS path: `hashlib.md5(f"{file_name}{utcnow}").hexdigest()[:12]`. Manual upload and pre-OCR'd upload: `secrets.token_hex(6)`. |
...
| `author` | `text`, nullable | GCS path: PDF metadata author if present. Manual upload and pre-OCR'd upload: always `""`. |
| `total_pages` | `integer` | PDF page count (PyMuPDF) or DOCX extracted-page count. Pre-OCR'd upload also uses the PyMuPDF page count, and rejects the request (400) if it doesn't match the number of pages in the `pages` payload. |
| `status` | `varchar(20)`, default `"pending"` | GCS path and manual PDF upload: `"pending"`. Manual DOCX upload and pre-OCR'd upload: `"ocr_done"` (OCR skipped). |
| `file_name` | `text`, nullable | Original uploaded filename. |
| `file_type` | `varchar(10)`, default `"pdf"` | Manual upload sets explicitly (`"pdf"`/`"docx"`); pre-OCR'd upload always sets `"pdf"`; GCS discovery relies on the column default since it only ever creates PDF books. |
| `source` | `text`, nullable | `"gcs_sync"` (GCS discovery), `"upload"` (manual upload), or `"surya_local"` (pre-OCR'd upload via `POST /books/upload-ocrd`, named for the local Surya OCR client that calls it). |
| `pipeline_step` | `varchar(20)`, nullable | GCS path and manual PDF upload: `NULL`. Manual DOCX upload and pre-OCR'd upload: `"chunking"` (`PIPELINE_STEP_CHUNKING`), since OCR is skipped. |
| `ocr_milestone` / `chunking_milestone` / `embedding_milestone` / `spell_check_milestone` | `varchar(20)`, default `"idle"` | Book-level milestones use a distinct vocabulary from page-level milestones: `idle \| in_progress \| complete \| partial_failure \| failed` (see `BOOK_MILESTONE_*` in `packages/backend-core/app/core/pipeline.py`), vs. pages' `idle \| in_progress \| succeeded \| failed`. GCS discovery leaves all four at the column default `"idle"`. Manual PDF upload explicitly sets `ocr_milestone="idle"` (the other three are also set to `"idle"`). Manual DOCX upload and pre-OCR'd upload both set `ocr_milestone="complete"` (a literal string, matching `BOOK_MILESTONE_COMPLETE` but not imported as a constant at either call site) since OCR is bypassed; the other three milestones are set to `"idle"`. |
| `cover_url` | `text`, nullable | `NULL` if extraction failed on any path. Otherwise it diverges: manual upload and pre-OCR'd upload both store the raw relative path `remote_cover_path` (`covers/{book_id}.jpg`); GCS discovery stores `storage.get_public_url(remote_cover_path)`, which resolves to `/api/covers/{book_id}.jpg` locally or `https://storage.googleapis.com/{bucket}/covers/{book_id}.jpg` on GCS. |
| `categories` | `text[]` | `[]` on creation from all three entry points. |
| `visibility` | `varchar(20)`, default `"private"` | Manual upload and pre-OCR'd upload both set it explicitly (`visibility="private"`). GCS discovery does not pass it to `create()` and relies on the column default. All paths land on the same value (`"private"`), but via different mechanisms. |
| `upload_date` / `last_updated` | `timestamptz` | Set to `datetime.now(timezone.utc)` by all three paths. |
| `created_by` / `updated_by` | `varchar(255)`, nullable | Manual upload and pre-OCR'd upload: the uploading user's email. GCS discovery leaves these `NULL`. |
...
| `page_number` | `integer` | `1..total_pages`, one row per PDF page (or per extracted DOCX page, or per entry in the pre-OCR'd `pages` payload). |
| `text` | `text`, nullable | PDF stubs: `NULL` (filled in by OCR). DOCX rows and pre-OCR'd rows: pre-filled — DOCX with the extracted page text, pre-OCR'd with `pages[n].text` from the request payload. |
| `is_toc` | `boolean`, default `false` | PDF stubs and DOCX rows: left at the column default `false`. Pre-OCR'd rows: explicitly set from `pages[n].isToc` in the request payload. |
| `status` | `varchar(20)`, default `"pending"` | PDF stubs: left at the column default `"pending"`. DOCX rows and pre-OCR'd rows: explicitly `"ocr_done"`. |
| `pipeline_step` | `varchar(20)`, nullable | PDF stubs: `NULL`. DOCX rows and pre-OCR'd rows: `"chunking"`. |
| `milestone` | `varchar(20)`, nullable | PDF stubs: `NULL`. DOCX rows and pre-OCR'd rows: `"idle"`. |
| `ocr_milestone` | `varchar(20)`, default `"idle"` | PDF stubs: left at the column default `"idle"`. DOCX rows and pre-OCR'd rows: explicitly `"succeeded"` (`PAGE_MILESTONE_SUCCEEDED`) since text is already present. |
| `chunking_milestone` / `embedding_milestone` / `spell_check_milestone` | `varchar(20)`, default `"idle"` | All three creation paths leave these at `"idle"` (PDF stubs via column default, DOCX and pre-OCR'd rows via explicit assignment). |

Row construction: `create_page_stubs(session, book_id, page_count)` in `packages/backend-core/app/services/pdf_service.py` builds the empty PDF stub rows (`Page(book_id=book_id, page_number=n)` for `n in range(1, page_count + 1)`); the DOCX branch in `upload_pdf` and the `upload_pdf_ocrd` endpoint both construct fully-populated `Page` objects directly instead of calling `create_page_stubs`.
...
| `packages/backend-core/app/services/storage_service.py` | Storage abstraction (`FileSystemStorageProvider` / `GCSStorageProvider`, selected by `STORAGE_BACKEND`) used by all three entry points to list, upload, and download files. |
| `packages/backend-core/app/services/pdf_service.py` | Shared PDF helpers used by all three entry points: `read_pdf_page_count`, `extract_pdf_cover`, `create_page_stubs`. |
...
| `packages/backend-core/app/db/repositories/books_repository.py` | `BooksRepository` — `find_by_hash`/`find_by_filename` duplicate lookups and `create()`, used by all three entry points. |
| `services/backend/api/endpoints/books_router.py` | Hosts `POST /books/upload` (`upload_pdf`) and `POST /books/upload-ocrd` (`upload_pdf_ocrd`), the two manual-request entry points. |
| `packages/backend-core/app/models/schemas.py` | `OcrPageInput` — the per-page shape (`pageNumber`, `text`, `isToc`) the `pages` payload of `POST /books/upload-ocrd` is validated against. |
...
flowchart TD
    subgraph Triggers [Entry Points]
        ...
        T3["Editor/Admin (Surya OCR client):<br/>POST /books/upload-ocrd<br/>(.pdf + pre-OCR'd pages)"]
    end
    ...
    T3 -->|"Hash file, SHA-256"| O_DUP{"content_hash<br/>already exists?"}
    O_DUP -->|Yes| O_EXIST(["Return existing bookId<br/>status: existing<br/>(no new row)"])
    O_DUP -->|No| O_COUNT{"pages payload count/<br/>numbers match PDF<br/>page count?"}
    O_COUNT -->|No| O_REJECT(["400 errors.pages_count_mismatch"])
    O_COUNT -->|Yes| O_CREATE["Create Book row +<br/>pre-filled Page rows"]

    U_CREATE --> InitDB(["Book + Page rows exist<br/>PDF: status=pending, ocr_milestone=idle<br/>DOCX / pre-OCR'd: status=ocr_done, ocr skipped<br/>GCS: status=pending, ocr_milestone=idle"])
    D_CREATE --> InitDB
    O_CREATE --> InitDB
    ...
    class InitDB done
    class U_CREATE,D_STD,D_DL,O_CREATE active
    class U_EXIST,D_SKIP,D_SKIPDUP,O_EXIST idle
    class O_REJECT fail
```
...
**Pre-OCR'd upload endpoint — `upload_pdf_ocrd` (`POST /books/upload-ocrd`):**

```
1. Parse the `pages` part, which the client may send as either a raw JSON
   string (multipart Form field) or a multipart file part (added by
   commit 4351976 so large books whose page JSON exceeds form-field size
   limits can still be pushed); either shape is decoded and validated
   against List[OcrPageInput] (pageNumber, text, isToc). Malformed JSON
   or a schema mismatch returns HTTPException(400,
   "errors.invalid_pages_payload").
2. Validate the filename extension is .pdf (case-insensitive); reject
   anything else with HTTPException(400, "errors.invalid_file_type").
3. Stream the upload to a temp path in 1 MB chunks, incrementally
   computing its SHA-256 hash (capped at settings.max_book_upload_bytes,
   same as the plain upload endpoint).
4. Look up an existing Book by that hash (find_by_hash). If found, delete
   the temp file and return {"bookId": existing.id, "status": "existing"}
   without creating anything new.
5. Read the PDF's page count (PyMuPDF) and cross-check it against the
   `pages` payload: both the count and the exact set of page numbers must
   match, or the temp file is deleted and HTTPException(400,
   "errors.pages_count_mismatch") is raised.
6. Generate book_id = secrets.token_hex(6), extract a cover, upload the
   PDF to uploads/{book_id}.pdf.
7. Create the Book row: status="ocr_done", pipeline_step="chunking",
   ocr_milestone="complete", source="surya_local" (OCR is skipped —
   the pages arrive already OCR'd by the external client).
8. Create one fully-populated Page row per payload entry (text, is_toc,
   ocr_milestone="succeeded") so chunking can start immediately.
9. Commit; bump the "books:list" and "category" cache namespaces; return
   {"bookId": book_id, "status": "uploaded"}.
```
...
| `MAX_BOOK_UPLOAD_BYTES` | `200 MiB` (`200 * 1024 * 1024`) | `settings.max_book_upload_bytes` — enforced by both `upload_pdf` and `upload_pdf_ocrd` while streaming the uploaded file to a temp path; exceeding it aborts with HTTP 413. Not enforced on GCS-discovered files (the scanner downloads whatever size is already in the bucket) or on the `pages` JSON part of `POST /books/upload-ocrd`. |
...
| `POST /books/upload-ocrd` | `Depends(require_editor)` (ADMIN or EDITOR) | Uploads a `.pdf` file plus a `pages` part (JSON string or file) of pre-OCR'd page text; validates page count/numbers against the PDF; dedupes by SHA-256 content hash; creates a `Book` row (`source="surya_local"`) and fully pre-filled `Page` rows, entering the pipeline at chunking. |
...
- No content/malware scanning of uploaded files was found in any path.
...
- `services/backend/tests/api/endpoints/books_router_test.py` covers both request-based endpoints: `test_upload_pdf_exceeds_size_limit` for `upload_pdf`'s `MAX_BOOK_UPLOAD_BYTES` check, and `test_upload_ocrd_rejects_page_count_mismatch`, `test_upload_ocrd_rejects_invalid_pages_json`, `test_upload_ocrd_returns_existing_book_on_duplicate_hash`, `test_upload_ocrd_creates_book_with_prefilled_pages_on_success`, and `test_upload_ocrd_with_uploadfile_pages_on_success` (covering both the JSON-string and multipart-file shapes of the `pages` part) for `upload_pdf_ocrd`. There is no dedicated happy-path test for `upload_pdf`'s successful-creation branch (PDF or DOCX) beyond the size-limit case.

## Related Docs

- [OCR_DESIGN.md](OCR_DESIGN.md) — next stage; begins once a page's `ocr_milestone` is `idle` (skipped for DOCX uploads and pre-OCR'd uploads via `POST /books/upload-ocrd`).
