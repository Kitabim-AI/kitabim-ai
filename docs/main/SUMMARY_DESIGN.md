...
See also: [WORKER_DESIGN.md](WORKER_DESIGN.md) for the full pipeline overview. Prior stages: [OCR_DESIGN.md](OCR_DESIGN.md), [CHUNKING_DESIGN.md](CHUNKING_DESIGN.md), [EMBEDDING_DESIGN.md](EMBEDDING_DESIGN.md), [SPELLCHECK_DESIGN.md](SPELLCHECK_DESIGN.md).
...
- **A summary failure never blocks book availability**, but the RAG-side consequence is more specific than "falls back to category-based search" — see the corrected description in Related Docs / Interfaces below (this doc replaces that stale claim from `WORKER_DESIGN.md`).
...
| `gemini_chat_model` (`system_configs`) | No fallback — required | `summary_job` — chat model used for summary generation; job raises if unset. |
| `gemini_embedding_model` (`system_configs`) | No fallback — required | `summary_job` — embedding model for the summary vector; job raises if unset. |
...
- CHAT_RAG_DESIGN.md (not yet written as of this doc) will document the `search_books_by_summary`/`get_book_summary` RAG agent tools and the "summary"/"identity" intent routing that consumes `book_summaries` as described above.
