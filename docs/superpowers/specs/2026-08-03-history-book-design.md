...
### Duplicate facts within one entry

Separate from duplicate *entries* above: a single entry's `facts` array can itself contain duplicate/near-duplicate facts (repeated extraction across overlapping book windows, pre-dating hardening of the merge pipeline, etc.) — these already exist in production data, not just a future risk.

- `POST /admin/history-dictionary/{id}/facts/merge` (new) — body `{keep_fact_id, remove_fact_id}`. Reuses `history_fact_utils.merge_citation` to fold `remove_fact_id`'s citations onto `keep_fact_id` (deduping any shared book, unioning page numbers), then removes `remove_fact_id`. Without this, deleting a duplicate outright (via the plain fact-delete endpoint above) would silently drop any citation unique to the removed fact. Updates `history_dictionary.facts`, resyncs via `materialize_history_facts`.
- **One-time cleanup script**, scoped to `is_ai_generated = TRUE` entries: for each entry, scans its `facts` array for duplicates using the *same* two-tier logic the extraction pipeline already uses at merge time (`history_fact_utils.find_deterministic_duplicate` for near-exact spelling-normalized matches, `cosine_similarity` for embedding-similar candidates):
  - **Tier 1 (deterministic) duplicates are auto-merged** — same merge logic as the endpoint above, safe to apply unsupervised since it's a near-exact spelling match.
  - **Tier 2 (embedding-similarity-only) candidates are flagged/logged for admin review, not auto-merged.** This is a deliberate asymmetry, not an oversight: this codebase has already had one production incident from an over-permissive automatic matcher silently discarding legitimate content (see `_build_strict_term_where`'s docstring) — a bulk script auto-merging on embedding similarity alone, with no LLM tier-3 adjudication and no human in the loop, risks the same failure mode at larger scale. Flagged candidates go through the manual merge endpoint above instead.
  - Idempotent/safe to re-run — after a run, remaining facts have no Tier-1 duplicates left, so a second run is a no-op on already-cleaned entries.
...
- Duplicate-fact cleanup tests: merge endpoint combines citations correctly (union of pages when both facts cite the same book, both citations kept when they cite different books) and removes the redundant fact; cleanup script auto-merges only Tier-1 (deterministic) duplicates; Tier-2 (embedding-only) candidates are flagged, not merged; re-running the script after a clean run is a no-op.
