...
- `HistoryExtractionService.process_book_pages` (`history_extraction_service.py:227-286`) sorts pages, slides a window of `batch_size` (default from `system_configs.history_batch_size`, fallback 15) with `overlap=2`, and calls `_call_llm_extraction` per window.
