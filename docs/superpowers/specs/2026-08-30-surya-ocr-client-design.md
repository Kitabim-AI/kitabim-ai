...
- **Correcting a page's text on an already-ingested book is also existing behavior, and already synchronous**: `POST /{book_id}/pages/{page_num}/update` updates `Page.text`, invalidates stale spell-check issues, deletes and recreates that page's chunks, re-embeds them inline via Gemini, and refreshes book-level milestones/cache — all in one call, no scanner round-trip needed. `POST /{book_id}/pages/{page_num}/toc` separately sets `Page.is_toc` (`PageTocUpdate` schema). This design's "correct an existing book" flow reuses both endpoints as-is instead of adding a new bulk one — see Scope.
...
- New: one Kitabim backend endpoint (`POST /books/upload-ocrd`) for ingesting a pre-OCR'd PDF as a new book. No new backend code for correcting an existing book — that flow reuses the existing `/pages/{page_num}/update` and `/pages/{page_num}/toc` endpoints.
...
services/backend/api/endpoints/books_router.py   # + 1 new endpoint (below)
```
...
### Correcting an existing book — no new endpoint

`POST /{book_id}/pages/{page_num}/update` (body: `{"text": "..."}`) and `POST /{book_id}/pages/{page_num}/toc` (body: `{"isToc": true|false}`, `PageTocUpdate` schema) already exist and already do everything a bulk override endpoint would need to, per page: overwrite `text`, invalidate stale spell-check issues, delete and synchronously recreate that page's chunks, re-embed them inline (Gemini), refresh book-level milestones, and invalidate the book/RAG cache. The client's correction flow calls these two endpoints once per corrected page instead of a new bulk endpoint — see Client Design.
...
- **Correcting an existing book:** `correct <book_id>` downloads the PDF via `GET /{book_id}/download`, runs the same local OCR/preview loop scoped to the pages the user picks, then for each changed page calls `POST /{book_id}/pages/{page_num}/update` followed by `POST /{book_id}/pages/{page_num}/toc` (sequentially per page, since `/update` already does synchronous chunk/embed work — no batching needed for what's expected to be a handful of pages at a time).
...
| Correction push targets a page number that doesn't exist on the book | The existing `/update`/`/toc` endpoints already 404 in this case; the client surfaces the error and stops (no partial application assumed for the pages that do exist). |

## Testing

- **Client (`clients/surya-ocr/`):** its own local pytest suite for the vendored engine/text-cleanup functions, the preview server's redo/multi-select logic, and the push logic (new-book and per-page correction), mocking the Kitabim API. No dependency on `backend-core`'s test fixtures.
- **Backend:** standard repository/service/endpoint coverage per `api-unit-tester` conventions for `upload-ocrd` only — page-count validation, content-hash dedup, correct `Page`/`Book` field values on success, auth gating. `/update` and `/toc` are pre-existing and already covered by their own tests; this design adds no new assertions there.
