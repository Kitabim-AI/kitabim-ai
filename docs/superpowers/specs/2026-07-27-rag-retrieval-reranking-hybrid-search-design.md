...
3. **One Gemini call.** Candidates are numbered `1..N` in the prompt (question + each candidate's text + book/page metadata for citation). The model returns a JSON array of chunk indices in relevance order. No re-derivation of book_id/page from model output — indices map straight back to the original candidate objects, so citation metadata can't be corrupted by the LLM.
   ...
   **Extraction correction (found in production):** the array is extracted from the raw response via `res_text.find("[")` + `json.JSONDecoder().raw_decode(res_text, start)`, not a bracket regex. An earlier version used `re.search(r"\[.*\]", res_text, re.DOTALL)`, which is greedy — it matches from the first `[` to the *last* `]` anywhere in the whole response. When the model violated the "output only JSON" instruction and appended trailing commentary that happened to contain another bracket further along (e.g. a citation-style `[reference]`), the regex over-captured through to that bracket, producing a "JSON: Extra data" parse error and an avoidable fallback to `_grade_context`. `raw_decode` parses only the first complete JSON value starting at `start` and ignores everything after it, regardless of what the trailing text contains — the correct tool for "parse one JSON value, don't care what's appended after."
