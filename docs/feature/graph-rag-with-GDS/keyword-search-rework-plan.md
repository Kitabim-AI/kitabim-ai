...
- [x] **3.6 Batch size = a new system config `sys_collection_page_size` (default 40)**, replacing the hardcoded `COLLECTION_PAGE_SIZE = 40` in `hooks/useBooks.ts`. Applies to the library shelves and the search results alike (one source of truth).
  - [x] Seeded `sys_collection_page_size` (default `40`) in `db/seeds.py`.
