...
flowchart TD
    ...
    class J_OCR,J_CH,J_EM,J_SC,J_SUM,J_KG,J_AC,J_GR job
    class OB,ED event
    class PD driver
    class PG,N4J,GQ db
...
Both feature-flagged off by default. When enabled, they replace the interactive-API branch of `OCR Job`/`Embedding Scanner` with an async submit-then-poll cycle against the Gemini Batch API. See [OCR_DESIGN.md](../../main/OCR_DESIGN.md#data-flow) for the batch-OCR diagram and [EMBEDDING_DESIGN.md](../../main/EMBEDDING_DESIGN.md#data-flow) for the batch-embedding diagram.
...
The exhaustion behavior differs by step — see [WORKER_DESIGN.md § PipelineDriver](../../main/WORKER_DESIGN.md#pipelinedriver) for the full rationale:
...
| **Graph resolution queue** | Postgres `graph_resolution_queue` — the handoff between graph extraction and entity resolution. `graph_resolution_scanner` claims rows every 5 min (`FOR UPDATE SKIP LOCKED`, oldest generation first) and dispatches one `graph_resolution_job` per scope; see [KNOWLEDGE_GRAPH_DESIGN.md](../../main/KNOWLEDGE_GRAPH_DESIGN.md) |
