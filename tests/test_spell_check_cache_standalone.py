    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "unique_to_book": {
                ...
                "hit_rate": self._stats["unique_hits"] / total_unique
                if total_unique > 0
                else 0,
            },
            ...
            "overall_hit_rate": (
                (
                    self._stats["unknown_hits"]
                    + self._stats["ocr_hits"]
                    + self._stats["unique_hits"]
                )
                / (total_unknown + total_ocr + total_unique)
                if (total_unknown + total_ocr + total_unique) > 0
                else 0
            ),
        }
...
async def test_statistics_tracking():
    ...
    ), f"Should have 50% overall hit rate (3 hits / 6 total), got {stats['overall_hit_rate']}"
