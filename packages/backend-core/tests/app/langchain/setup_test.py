...
def test_configure_langchain_cache_fail():
    with patch("logging.getLogger"):
        with patch("app.langchain.setup.settings") as mock_settings:
            mock_settings.langchain_tracing_enabled = False
            mock_settings.langchain_cache_enabled = True
            with patch("app.langchain.setup.log_json") as mock_log_json:
                with patch("langchain_core.globals.set_llm_cache", side_effect=Exception("fail")):
                    configure_langchain()
                    assert mock_log_json.called
