...
async def test_get_usage_redis_miss_db_hit(chat_limit_service):
    ...
    with patch("app.services.cache_service.cache_service.get", return_value=None):
        with patch(
            "app.db.repositories.user_chat_usage_repository.UserChatUsageRepository.get_usage",
            return_value=7,
        ):
            with patch(
                "app.services.cache_service.cache_service.set", return_value=None
            ) as mock_set:
                usage = await chat_limit_service.get_usage("user-123", session)
                assert usage == 7
...
async def test_increment_usage(chat_limit_service, mock_user):
    session = AsyncMock()
    with patch(
        "app.db.repositories.user_chat_usage_repository.UserChatUsageRepository.increment_usage",
        return_value=6,
    ):
        with patch(
            "app.services.cache_service.cache_service.get_client",
