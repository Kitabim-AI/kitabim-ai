...
async def test_get_limit_for_role_reader(chat_limit_service):
    session = AsyncMock()
    with patch("app.db.repositories.system_configs_repository.SystemConfigsRepository.get_value", return_value="50"):
...
async def test_get_limit_fallback(chat_limit_service):
    session = AsyncMock()
    # Mock exception
    with patch("app.db.repositories.system_configs_repository.SystemConfigsRepository.get_value", side_effect=Exception("fail")):
...
async def test_get_usage_redis_miss_db_hit(chat_limit_service):
    ...
    with patch("app.services.cache_service.cache_service.get", return_value=None):
        with patch("app.db.repositories.user_chat_usage_repository.UserChatUsageRepository.get_usage", return_value=7):
...
async def test_increment_usage(chat_limit_service, mock_user):
    session = AsyncMock()
    with patch("app.db.repositories.user_chat_usage_repository.UserChatUsageRepository.increment_usage", return_value=6):
