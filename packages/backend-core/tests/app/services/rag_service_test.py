...
async def test_grade_context_node_preserves_metadata(monkeypatch):
    ...
    with patch("app.services.rag.agent.nodes.grade_context_node.get_stream_writer", return_value=mock_writer):
        # Setup observations
...
async def test_execute_tool_node_handles_failure():
    ...
    with patch("app.services.rag.agent.nodes.execute_tool_node.get_stream_writer", return_value=mock_writer), \
