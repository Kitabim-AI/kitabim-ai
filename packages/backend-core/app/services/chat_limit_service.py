    ...
    async def get_limit_for_role(
        ...
        try:
            ...
            log_json(
                logger,
                logging.WARNING,
                "Failed to fetch chat limit from DB",
                key=limit_key,
                error=str(exc),
            )
    ...
    async def get_usage(self, user_id: str, session: AsyncSession) -> int:
        ...
        try:
            ...
            await cache_service.set(
                cache_key, current_usage, ttl=seconds_until_midnight
            )
        except Exception as e:
    ...
    async def increment_usage(self, user: User, session: AsyncSession) -> int:
        ...
        if user.role == UserRole.ADMIN:
            log_json(
                logger,
                logging.DEBUG,
                "Skipping usage increment for admin",
                user_id=user.id,
            )
            return 0
        ...
        try:
            ...
            if client:
                new_count_redis = await client.eval(
                    INCR_EXPIRE_LUA, 1, cache_key, seconds_until_midnight
                )
                log_json(
                    logger,
                    logging.INFO,
                    "Usage incremented in Redis",
                    user_id=user.id,
                    new_count=new_count_redis,
                )
        except Exception as e:
