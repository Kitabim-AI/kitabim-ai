...
def read_pdf_page_count(path: Path) -> int:
    ...
    try:
        ...
        logger.warning(
            "pdf_service: failed to read page count path=%s error=%s", path, e
        )
        return 0
...
def extract_pdf_cover(pdf_path: Path, cover_path: Path) -> bool:
    ...
    try:
        ...
        logger.warning(
            "pdf_service: failed to extract cover path=%s error=%s", pdf_path, e
        )
    return False
...
def create_page_stubs(session: AsyncSession, book_id: str, page_count: int) -> None:
    ...
    if page_count > 0:
        session.add_all(
            [Page(book_id=book_id, page_number=n) for n in range(1, page_count + 1)]
        )
