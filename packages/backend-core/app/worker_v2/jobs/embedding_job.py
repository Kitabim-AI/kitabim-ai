...
logger = logging.getLogger("app.worker_v2.embedding_job")

EMBED_BATCH_SIZE = 20


async def v2_embedding_job(ctx, page_ids: List[int]) -> None:
    log_json(logger, logging.INFO, "V2 embedding job started", page_count=len(page_ids))
    ...
    async with db_session.async_session_factory() as session:
        await session.execute(
            ...
            .values(v2_pipeline_step="embedding")
        )
    ...
    for page in pages:
        try:
            ...
            succeeded += 1
            log_json(logger, logging.DEBUG, "V2 embedding page succeeded",
            ...
            failed += 1
            log_json(logger, logging.WARNING, "V2 embedding page failed",
                     book_id=page.book_id, page=page.page_number, error=str(exc))

    log_json(logger, logging.INFO, "V2 embedding job completed",
