...
logger = logging.getLogger("app.worker_v2.chunking_scanner")
...
async def run_chunking_scanner(ctx) -> None:
    ...
    await redis.enqueue_job("v2_chunking_job", page_ids=page_ids)
    log_json(logger, logging.INFO, "V2 chunking job dispatched", page_count=len(page_ids))
