...
logger = logging.getLogger("app.worker_v2.embedding_scanner")
...
async def run_embedding_scanner(ctx) -> None:
    ...
    async with db_session.async_session_factory() as session:
        ...
        page_limit = int(await config_repo.get_value("v2_scanner_page_limit", "100"))
    ...
    await redis.enqueue_job("v2_embedding_job", page_ids=page_ids)
    log_json(logger, logging.INFO, "V2 embedding job dispatched", page_count=len(page_ids))
