...
def fix_malformed_citations(text: str) -> str:
    ...
    def replace_pattern1(match):
        ...
        if pages:
            ...
            logger.info(
                f"Fixed malformed citation: ref:{book_id}]بەت{pages_str}{citation_text} -> [{full_citation}](ref:{book_id}:{pages})"
            )
            return result
    ...
    def replace_pattern2(match):
        ...
        logger.info(
            f"Fixed standalone reference: ref:{book_id}:{pages} -> [مەنبە](ref:{book_id}:{pages})"
        )
        return result
